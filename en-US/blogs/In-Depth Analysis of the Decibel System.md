...
本说明文档旨在系统阐述分贝体系的数学原理、工程意义、不同参考框架与具体应用。本版本修正了所有数学公式，全部使用 Obsidian 支持的 LaTeX 数学环境（`$...$` 或 `$$...$$`），并增加了大量基础解释和例题。
...
**解：**
$G_{P,\mathrm{dB}} = 10 \log_{10}\left( \frac{1}{1000} \right) = 10 \times (-3) = -30 \mathrm{dB}$。负分贝值表示衰减或损耗。
...
推导过程：
1. 功率比： $$\frac{P_1}{P_0} = \left(\frac{A_1}{A_0}\right)^2$$
2. 代入功率分贝公式：
   $$G_{P,\mathrm{dB}} = 10 \log_{10}\!\left( \frac{P_1}{P_0} \right) = 10 \log_{10}\!\left( \left(\frac{A_1}{A_0}\right)^2 \right)$$
3. 利用对数性质： $$G_{P,\mathrm{dB}} = 10 \times 2 \times \log_{10}\!\left( \frac{A_1}{A_0} \right) = 20 \log_{10}\!\left( \frac{A_1}{A_0} \right)$$
...
**注意：** 虽然此例中电压增益和功率增益都是 $20 \mathrm{dB}$，但这仅在阻抗相同的条件下成立。如果阻抗不同，$20 \log_{10}(V_{out}/V_{in})$ 给出的电压增益分贝值，并不等于实际的功率增益分贝值。
...
**解：**
$L_p = 20 \log_{10}\!\left( \frac{0.02}{2 \times 10^{-5}} \right) = 20 \log_{10}(1000) = 20 \times 3 = 60 \,\mathrm{dB \, SPL}$

**例题 3.2：** 喷气式飞机附近的声压约为 $200 \,\mathrm{Pa}$，求其声压级。
**解：**
$L_p = 20 \log_{10}\!\left( \frac{200}{2 \times 10^{-5}} \right) = 20 \log_{10}(10^7) = 20 \times 7 = 140 \,\mathrm{dB \, SPL}$
...
**解：**
$$A_{\mathrm{dBFS}} = 20 \log_{10}\left( \frac{1}{2} \right) = 20 \times (-0.3010) \approx -6.02 \,\mathrm{dBFS}$$
...
- **情况一：单极性方波（在 $0$ 与 $A_{\mathrm{peak}}$ 之间跳变）**  
  ...
  $$
  - **特例：50%占空比**时，$A_{\mathrm{RMS}} = A_{\mathrm{peak}}/\sqrt{2} = 0.707 \times A_{\mathrm{peak}}$，**与正弦波相同**。
  - **特例：100%占空比**时，$A_{\mathrm{RMS}} = A_{\mathrm{peak}}$（即为直流信号）。
...
$$
\begin{aligned}
A_{\mathrm{RMS},\mathrm{dBFS}} &= 20\log_{10}\!\left(\frac{A_{\mathrm{RMS}}}{A_{\max}}\right) \\
&= 20\log_{10}\!\left(\frac{A_{\mathrm{peak}}}{A_{\max}} \times \frac{1}{\sqrt{2}}\right) \\
&= 20\log_{10}\!\left(\frac{A_{\mathrm{peak}}}{A_{\max}}\right) + 20\log_{10}\!\left(\frac{1}{\sqrt{2}}\right) \\
&= -3 - 3.01 \\
&\approx -6.01 \,\mathrm{dBFS}.
\end{aligned}
$$
...
$$
\begin{aligned}
G_{P,\mathrm{dB}} &= 10 \log_{10}(P_{\mathrm{out}}/P_{\mathrm{in}}) \\
&= 10 \log_{10}((V_{\mathrm{out}}^2/R) / (V_{\mathrm{in}}^2/R)) \\
&= 10 \log_{10}((V_{\mathrm{out}}/V_{\mathrm{in}})^2) \\
&= 20 \log_{10}(V_{\mathrm{out}}/V_{\mathrm{in}}) \\
&= G_{V,\mathrm{dB}}
\end{aligned}
$$
...
**解：**
$$P_{in} = V_{in}^2 / R_{in} = 1^2 / 50 = 0.02 \mathrm{W}$$
$$P_{out} = V_{out}^2 / R_{out} = 10^2 / 500 = 0.2 \mathrm{W}$$
$$G_{P,\mathrm{dB}} = 10 \log_{10}(0.2 / 0.02) = 10 \log_{10}(10) = 10 \mathrm{dB}$$
...
电压增益（dB）为：
$$
A_{v,\mathrm{dB}} = 20\log_{10}\!\left|\frac{R_f}{R_{\mathrm{in}}}\right| = 20\log_{10}(A_v) \quad (\text{因为取绝对值})
$$

**例题 5.2：** 反相放大器中，$R_{in} = 1\mathrm{k\Omega}$，$R_f = 10\mathrm{k\Omega}$，求 $A_v$ 和 $A_{v,\mathrm{dB}}$。
**解：**
$$A_v = -10\mathrm{k\Omega} / 1\mathrm{k\Omega} = -10$$
...
**解：**
$$A_v = 1 + 9\mathrm{k\Omega} / 1\mathrm{k\Omega} = 10$$
$$A_{v,\mathrm{dB}} = 20\log_{10}(10) = 20 \mathrm{dB}$$
...
**解：**
$$f_{\mathrm{BW}} \approx \mathrm{GBW} / A_{\mathrm{CL}} = 1\mathrm{MHz} / 100 = 10 \mathrm{kHz}$$
...
$$
V_{\mathrm{RMS}} = \frac{V_{\text{pk}}}{\sqrt{2}} \quad\Longrightarrow\quad V_{\text{pk}} = V_{\mathrm{RMS}}\sqrt{2}
$$
...
**例题 6.1：**
- $P=1\,\mathrm{mW} \Rightarrow P_{\mathrm{dBm}}=0\,\mathrm{dBm}$
...
$$
V_{0.775\mathrm{V}} = \sqrt{P \cdot R} = \sqrt{0.001\,\mathrm{W} \times 600\,\Omega} \approx 0.7745967\,\mathrm{V}
$$
...
$$
\text{dBu} = \text{dBV} + 20\log_{10}\!\left(\frac{1}{0.775}\right) \approx \text{dBV} + 2.214\,\mathrm{dB}
$$
等价地：
$$
...
**例题 6.2：**
- $0\,\mathrm{dBV}$ 是多少 $\mathrm{dBu}$？ $\text{dBu} = 0 + 2.214 = +2.214\,\mathrm{dBu}$
- $0\,\mathrm{dBu}$ 是多少 $\mathrm{dBV}$？ $\text{dBV} = 0 - 2.214 = -2.214\,\mathrm{dBV}$
- 专业设备常见的线路电平是 $+4\,\mathrm{dBu}$，这相当于多少 $\mathrm{dBV}$ 和 $V_{\mathrm{RMS}}$？
  $\text{dBV} = +4 - 2.214 = +1.786\,\mathrm{dBV}$
  $V_{\mathrm{RMS}} = 0.775 \times 10^{(4/20)} = 0.775 \times 10^{0.2} \approx 0.775 \times 1.5849 \approx 1.228\,\mathrm{V}$
- 消费级设备常见的线路电平是 $-10\,\mathrm{dBV}$，这相当于多少 $\mathrm{dBu}$ 和 $V_{\mathrm{RMS}}$？
  $\text{dBu} = -10 + 2.214 = -7.786\,\mathrm{dBu}$
  $V_{\mathrm{RMS}} = 1 \times 10^{(-10/20)} = 10^{-0.5} \approx 0.3162\,\mathrm{V}$
...
- 在 $50\,\Omega$ 系统（射频标准）中：
  $P_{\mathrm{dBm}} = 20\log_{10}(V) - 10\log_{10}(50) + 30 = 20\log_{10}(V) - 16.99 + 30 \approx 20\log_{10}(V) + 13.01$
- 在 $75\,\Omega$ 系统（电视标准）中：
  $P_{\mathrm{dBm}} = 20\log_{10}(V) - 10\log_{10}(75) + 30 = 20\log_{10}(V) - 18.75 + 30 \approx 20\log_{10}(V) + 11.25$
- 在 $600\,\Omega$ 系统（传统音频标准）中：
  $P_{\mathrm{dBm}} = 20\log_{10}(V) - 10\log_{10}(600) + 30 = 20\log_{10}(V) - 27.78 + 30 \approx 20\log_{10}(V) + 2.22$
  注意，当 $V=0.775\mathrm{V}$ 时，$P_{\mathrm{dBm}} \approx 20\log_{10}(0.775) + 2.22 \approx -2.22 + 2.22 = 0\mathrm{dBm}$，与定义一致。

**例题 6.3：** 在 $50\Omega$ 系统中，测得正弦波信号 $V_{\mathrm{RMS}} = 1\mathrm{V}$，求功率 $P_{\mathrm{dBm}}$。
**解：**
$P_{\mathrm{dBm}} \approx 20\log_{10}(1) + 13.01 = 0 + 13.01 = +13.01\,\mathrm{dBm}$
或用功率直接算：$P = V^2/R = 1/50 = 0.02\mathrm{W} = 20\mathrm{mW}$, $P_{\mathrm{dBm}} = 10\log_{10}(20) \approx 13.01\mathrm{dBm}$。
...
| $\mathbf{0\,\mathrm{dBm}}$ (50Ω) | $1\,\mathrm{mW}$ | $0.224\,\mathrm{V}$ | $0.316\,\mathrm{V}$ | $-7.78\,\mathrm{dBm}$ | $0\,\mathrm{dBm}$ | 射频系统功率 |
...
计算常数项：$$20\log_{10}\left( \frac{4\pi}{c} \right) \approx 20\log_{10}(4.1888\times10^{-8}) \approx -147.55$$
...
**解：**
$d=100\mathrm{m}$, $f=2.4\times10^9\mathrm{Hz}$
$\mathrm{FSPL} = 20\log_{10}(100) + 20\log_{10}(2.4\times10^9) - 147.55$
$= 40 + 187.6 - 147.55 \approx 80.05\mathrm{dB}$
